...
function createTempFile(filename: string, content: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'linkos-test-'));
...
describe('file sending: real Telegram API', () => {
  ...
  beforeEach(() => {
    tmpFile = createTempFile('linkos-test.txt', `LinkOS file send test\nTimestamp: ${new Date().toISOString()}\nThis file was sent by an automated integration test.`);
  });
  ...
  it.skipIf(!canRunRealTests)('sends a real document via Telegram sendDocument API', async () => {
    ...
    formData.append('document', new Blob([fs.readFileSync(tmpFile)]), 'linkos-test.txt');
    ...
    expect(json.result?.document?.file_name).toBe('linkos-test.txt');
  }, 15000);
...
%%EOF`;
    ...
    const pdfFile = createTempFile('linkos-test.pdf', pdfContent);
    try {
      const formData = new FormData();
      formData.append('chat_id', chatId);
      formData.append('document', new Blob([fs.readFileSync(pdfFile)]), 'linkos-test.pdf');
      ...
      expect(json.ok).toBe(true);
      expect(json.result?.document?.file_name).toBe('linkos-test.pdf');
    } finally {
  ...
  it.skipIf(!canRunRealTests)('returns error for nonexistent file (sanity check)', async () => {
    ...
    const fakePath = '/tmp/linkos-nonexistent-test-file.pdf';
