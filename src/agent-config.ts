...
import { LINKOS_CONFIG, PROJECT_ROOT } from './config.js';
...
/**
 * Resolve the directory for a given agent, checking LINKOS_CONFIG first,
 * then falling back to PROJECT_ROOT/agents/<id>.
 */
export function resolveAgentDir(agentId: string): string {
  const externalDir = path.join(LINKOS_CONFIG, 'agents', agentId);
...
/**
 * Resolve the CLAUDE.md path for a given agent, checking LINKOS_CONFIG first,
 * then falling back to PROJECT_ROOT/agents/<id>/CLAUDE.md.
 */
export function resolveAgentClaudeMd(agentId: string): string | null {
  const externalPath = path.join(LINKOS_CONFIG, 'agents', agentId, 'CLAUDE.md');
 ...
 *  Scans both LINKOS_CONFIG/agents/ and PROJECT_ROOT/agents/, deduplicating.
 */
