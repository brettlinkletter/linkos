...
/** Run a read-only query on the database. For use by oracle/dashboard. */
...
function createSchema(database: Database.Database): void {
  database.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_builder_messages_project ON builder_messages(project_id, created_at);
    ...
    CREATE INDEX IF NOT EXISTS idx_builder_versions_project ON builder_versions(project_id, created_at DESC);

    -- ── Dev Projects (project tracker) ────────────────────────────────
    ...
    CREATE INDEX IF NOT EXISTS idx_dev_updates_project ON dev_project_updates(project_id, created_at DESC);
...
export function initDatabase(): void {
  ...
  const dbPath = path.join(STORE_DIR, 'linkos.db');
...
export function decayMemories(): void {
  ...
  `).run(oneDayAgo);
  // Clear FK references before deleting so superseded_by doesn't block the delete
  db.prepare(`
...
export function getDashboardTokenStats(chatId: string): DashboardTokenStats {
  const chatFilter = chatId ? 'chat_id = ?' : '1=1';
  ...
  const today = db
    .prepare(
      `SELECT
         ...
         COUNT(*) as todayTurns
       FROM token_usage
       WHERE ${chatFilter} AND created_at >= unixepoch('now', 'start of day')`,
    )
    .get(...chatParams) as { todayInput: number; todayOutput: number; todayCost: number; todayTurns: number };
  ...
  const allTime = db
    .prepare(
      `SELECT
       ...
       FROM token_usage WHERE ${chatFilter}`,
    )
    .get(...chatParams) as { allTimeInput: number; allTimeOutput: number; allTimeCost: number; allTimeTurns: number };

  return { ...today, ...allTime };
}

export function getDashboardCostTimeline(chatId: string, days = 30): { date: string; cost: number; turns: number }[] {
  const chatFilter = chatId ? 'chat_id = ? AND' : '';
  const chatParams = chatId ? [chatId, `-${days} days`] : [`-${days} days`];
  return db
    .prepare(
      `SELECT
         date(created_at, 'unixepoch') as date,
         ...
         COUNT(*) as turns
       FROM token_usage
       WHERE ${chatFilter} created_at >= unixepoch('now', ?)
       GROUP BY date
       ORDER BY date`,
    )
    .all(...chatParams) as { date: string; cost: number; turns: number }[];
}
...
export function getDashboardRecentTokenUsage(chatId: string, limit = 20): RecentTokenUsageRow[] {
  ...
  const chatParams = chatId ? [chatId, limit] : [limit];
  return db
    .prepare(
      `SELECT * FROM token_usage ${chatFilter}
       ORDER BY created_at DESC LIMIT ?`,
    )
    .all(...chatParams) as RecentTokenUsageRow[];
}
...
export interface KnowledgeGraphLink {
  source: string;
  target: string;
  type: 'has_entity' | 'has_topic' | 'shares_entity' | 'shares_topic' | 'connection';
  label?: string;
}
...
export function getKnowledgeGraphData(chatId: string): KnowledgeGraphData {
  const memories = db
    .prepare('SELECT * FROM memories WHERE chat_id = ? ORDER BY created_at DESC')
    .all(chatId) as Memory[];

  const nodes: KnowledgeGraphNode[] = [];
  const links: KnowledgeGraphLink[] = [];
  const nodeIds = new Set<string>();
  const entityMemories = new Map<string, string[]>(); // entity -> [memoryNodeIds]
  const topicMemories = new Map<string, string[]>();   // topic -> [memoryNodeIds]
  ...
  for (const mem of memories) {
    ...
    nodes.push({
      id: memNodeId,
      label: mem.summary.length > 60 ? mem.summary.slice(0, 57) + '...' : mem.summary,
      type: 'memory',
      agent: mem.agent_id,
    ...
    nodeIds.add(memNodeId);

    // Parse entities
    let entities: string[] = [];
    try { entities = JSON.parse(mem.entities || '[]'); } catch {}
    for (const ent of entities) {
      const entNodeId = `ent-${ent.toLowerCase().replace(/[^a-z0-9]+/g, '_')}`;
      if (!nodeIds.has(entNodeId)) {
        nodes.push({ id: entNodeId, label: ent, type: 'entity' });
        nodeIds.add(entNodeId);
      }
      links.push({ source: memNodeId, target: entNodeId, type: 'has_entity' });
      if (!entityMemories.has(entNodeId)) entityMemories.set(entNodeId, []);
      entityMemories.get(entNodeId)!.push(memNodeId);
    }

    // Parse topics
    let topics: string[] = [];
    try { topics = JSON.parse(mem.topics || '[]'); } catch {}
    for (const top of topics) {
      const topNodeId = `top-${top.toLowerCase().replace(/[^a-z0-9]+/g, '_')}`;
      if (!nodeIds.has(topNodeId)) {
        nodes.push({ id: topNodeId, label: top, type: 'topic' });
        nodeIds.add(topNodeId);
      }
      links.push({ source: memNodeId, target: topNodeId, type: 'has_topic' });
      if (!topicMemories.has(topNodeId)) topicMemories.set(topNodeId, []);
      topicMemories.get(topNodeId)!.push(memNodeId);
    }

    // Parse explicit connections
    let connections: { linked_to: number; relationship: string }[] = [];
    try { connections = JSON.parse(mem.connections || '[]'); } catch {}
    for (const conn of connections) {
      const targetMemId = `mem-${conn.linked_to}`;
      links.push({
        source: memNodeId,
        target: targetMemId,
        type: 'connection',
        label: conn.relationship.length > 60 ? conn.relationship.slice(0, 57) + '...' : conn.relationship,
      });
    }
  }

  // Add implicit links: memories sharing the same entity
  for (const [, memIds] of entityMemories) {
    if (memIds.length > 1) {
      for (let i = 0; i < memIds.length; i++) {
        for (let j = i + 1; j < memIds.length; j++) {
          links.push({ source: memIds[i], target: memIds[j], type: 'shares_entity' });
        }
      }
    }
  }

  // Add implicit links: memories sharing the same topic
  for (const [, memIds] of topicMemories) {
    if (memIds.length > 1) {
      for (let i = 0; i < memIds.length; i++) {
        for (let j = i + 1; j < memIds.length; j++) {
          links.push({ source: memIds[i], target: memIds[j], type: 'shares_topic' });
        }
      }
    }
  }

  return { nodes, links, agents: Array.from(agents) };
}
...
// ── Agent Status (cross-process Telegram liveness) ────────────────
...
export function upsertAgentStatus(agentId: string, data: {
  ...
  const now = Math.floor(Date.now() / 1000);
  db.prepare(`
    INSERT INTO agent_status (agent_id, telegram_connected, pid, bot_username, last_heartbeat, started_at)
    VALUES (?, ?, ?, ?, ?, ?)
    ON CONFLICT(agent_id) DO UPDATE SET
      telegram_connected = excluded.telegram_connected,
      pid = excluded.pid,
      bot_username = COALESCE(excluded.bot_username, agent_status.bot_username),
      last_heartbeat = excluded.last_heartbeat,
      started_at = CASE WHEN excluded.telegram_connected = 1 AND agent_status.telegram_connected = 0
                        THEN excluded.last_heartbeat ELSE agent_status.started_at END
  `).run(agentId, data.telegram_connected ? 1 : 0, data.pid, data.bot_username ?? null, now, now);
}

export function updateAgentHeartbeat(agentId: string): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare('UPDATE agent_status SET last_heartbeat = ? WHERE agent_id = ?').run(now, agentId);
}

export function clearAgentStatus(agentId: string): void {
  db.prepare('UPDATE agent_status SET telegram_connected = 0 WHERE agent_id = ?').run(agentId);
}

export function getAllAgentStatuses(): AgentStatusRow[] {
  return db.prepare('SELECT * FROM agent_status').all() as AgentStatusRow[];
}
...
// ── Personal Pulse Cache ──────────────────────────────────────────────

export function getPersonalPulseCache(key: string): { data: string; updated_at: number } | undefined {
  return db.prepare('SELECT data, updated_at FROM personal_pulse_cache WHERE key = ?').get(key) as { data: string; updated_at: number } | undefined;
}

export function setPersonalPulseCache(key: string, data: string): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare('INSERT OR REPLACE INTO personal_pulse_cache (key, data, updated_at) VALUES (?, ?, ?)').run(key, data, now);
}
...
export function createBuilderProject(id: string, name: string, description = ''): BuilderProject {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    'INSERT INTO builder_projects (id, name, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
  ).run(id, name, description, now, now);
  return { id, name, description, created_at: now, updated_at: now };
}

export function listBuilderProjects(): BuilderProject[] {
  return db.prepare('SELECT * FROM builder_projects ORDER BY updated_at DESC').all() as BuilderProject[];
}

export function getBuilderProject(id: string): BuilderProject | null {
  return (db.prepare('SELECT * FROM builder_projects WHERE id = ?').get(id) as BuilderProject) ?? null;
}

export function deleteBuilderProject(id: string): boolean {
  const txn = db.transaction(() => {
    db.prepare('DELETE FROM builder_files WHERE project_id = ?').run(id);
    db.prepare('DELETE FROM builder_messages WHERE project_id = ?').run(id);
    db.prepare('DELETE FROM builder_versions WHERE project_id = ?').run(id);
    const r = db.prepare('DELETE FROM builder_projects WHERE id = ?').run(id);
    return r.changes > 0;
  });
  return txn();
}

export function getBuilderFiles(projectId: string): BuilderFile[] {
  return db.prepare('SELECT * FROM builder_files WHERE project_id = ? ORDER BY file_path').all(projectId) as BuilderFile[];
}

export function saveBuilderFile(projectId: string, filePath: string, content: string): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    `INSERT INTO builder_files (project_id, file_path, content, updated_at) VALUES (?, ?, ?, ?)
  ...
  db.prepare('UPDATE builder_projects SET updated_at = ? WHERE id = ?').run(now, projectId);
}

export function saveBuilderFiles(projectId: string, files: Array<{ path: string; content: string }>): void {
  const now = Math.floor(Date.now() / 1000);
  const txn = db.transaction(() => {
    ...
    for (const f of files) {
      stmt.run(projectId, f.path, f.content, now);
    }
    db.prepare('UPDATE builder_projects SET updated_at = ? WHERE id = ?').run(now, projectId);
  });
  txn();
...
export function getBuilderMessages(projectId: string, limit = 50): BuilderMessage[] {
  return db.prepare(
    'SELECT * FROM builder_messages WHERE project_id = ? ORDER BY created_at ASC LIMIT ?',
  ).all(projectId, limit) as BuilderMessage[];
}

export function saveBuilderMessage(projectId: string, role: string, content: string): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    'INSERT INTO builder_messages (project_id, role, content, created_at) VALUES (?, ?, ?, ?)',
  ).run(projectId, role, content, now);
}

export function createBuilderVersion(projectId: string, message = ''): BuilderVersion | null {
  const files = getBuilderFiles(projectId);
  if (files.length === 0) return null;
  const id = crypto.randomBytes(8).toString('hex');
  const now = Math.floor(Date.now() / 1000);
  const maxV = (db.prepare('SELECT MAX(version) as v FROM builder_versions WHERE project_id = ?').get(projectId) as { v: number | null })?.v ?? 0;
  const version = maxV + 1;
  const snapshot = JSON.stringify(files.map((f) => ({ path: f.file_path, content: f.content })));
  db.prepare(
    'INSERT INTO builder_versions (id, project_id, version, snapshot, message, created_at) VALUES (?, ?, ?, ?, ?, ?)',
  ).run(id, projectId, version, snapshot, message, now);
  return { id, project_id: projectId, version, snapshot, message, created_at: now };
}

export function listBuilderVersions(projectId: string): BuilderVersion[] {
  return db.prepare(
    'SELECT id, project_id, version, message, created_at FROM builder_versions WHERE project_id = ? ORDER BY version DESC',
  ).all(projectId) as BuilderVersion[];
}

export function restoreBuilderVersion(projectId: string, versionId: string): boolean {
  const ver = db.prepare('SELECT * FROM builder_versions WHERE id = ? AND project_id = ?').get(versionId, projectId) as BuilderVersion | undefined;
  if (!ver) return false;
  const files = JSON.parse(ver.snapshot) as Array<{ path: string; content: string }>;
  const now = Math.floor(Date.now() / 1000);
  const txn = db.transaction(() => {
...
export interface DevProjectUpdate {
  id: number;
  project_id: string;
  agent_id: string;
  type: string;
...
export function listDevProjects(): DevProject[] {
  return db.prepare('SELECT * FROM dev_projects ORDER BY sort_order ASC, updated_at DESC').all() as DevProject[];
}

export function getDevProject(id: string): DevProject | null {
  return (db.prepare('SELECT * FROM dev_projects WHERE id = ?').get(id) as DevProject) ?? null;
}

export function createDevProject(data: {
  id: string; name: string; description?: string; repo?: string;
  status?: string; environment?: string; vercel_url?: string; prod_url?: string;
  branch?: string; tags?: string[]; screenshot?: string; notes?: string;
}): DevProject {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    `INSERT INTO dev_projects (id, name, description, repo, status, environment, vercel_url, prod_url, branch, tags, screenshot, notes, sort_order, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    data.id, data.name, data.description || '', data.repo || '',
    data.status || 'planning', data.environment || 'local',
    data.vercel_url || '', data.prod_url || '', data.branch || '',
    JSON.stringify(data.tags || []), data.screenshot || '', data.notes || '',
    0, now, now,
  );
  return getDevProject(data.id)!;
}

export function updateDevProject(id: string, data: Partial<Omit<DevProject, 'id' | 'created_at'>>): DevProject | null {
  ...
  values.push(now);
  values.push(id);
  db.prepare(`UPDATE dev_projects SET ${fields.join(', ')} WHERE id = ?`).run(...values);
  return getDevProject(id);
}

export function deleteDevProject(id: string): boolean {
  const txn = db.transaction(() => {
    db.prepare('DELETE FROM dev_project_updates WHERE project_id = ?').run(id);
    const r = db.prepare('DELETE FROM dev_projects WHERE id = ?').run(id);
    return r.changes > 0;
  });
  return txn();
}

export function addDevProjectUpdate(projectId: string, agentId: string, type: string, content: string, metadata: Record<string, unknown> = {}): void {
  const now = Math.floor(Date.now() / 1000);
  db.prepare(
    'INSERT INTO dev_project_updates (project_id, agent_id, type, content, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?)',
  ).run(projectId, agentId, type, content, JSON.stringify(metadata), now);
  db.prepare('UPDATE dev_projects SET updated_at = ? WHERE id = ?').run(now, projectId);
}

export function getDevProjectUpdates(projectId: string, limit = 50): DevProjectUpdate[] {
  return db.prepare(
    'SELECT * FROM dev_project_updates WHERE project_id = ? ORDER BY created_at DESC LIMIT ?',
  ).all(projectId, limit) as DevProjectUpdate[];
}

export function getRecentDevActivity(limit = 30): (DevProjectUpdate & { project_name: string })[] {
  return db.prepare(
    `SELECT u.*, p.name as project_name FROM dev_project_updates u
     JOIN dev_projects p ON p.id = u.project_id
     ORDER BY u.created_at DESC LIMIT ?`,
  ).all(limit) as (DevProjectUpdate & { project_name: string })[];
}
