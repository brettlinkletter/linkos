 ...
 * Stops all LinkOS services and force-exits after a brief timeout.
 */
export function executeEmergencyKill(): void {
  ...
  try {
    if (os.platform() === 'darwin') {
      // Stop all LinkOS launchd services
      try {
        ...
        for (const line of output.split('\n')) {
          ...
          if (label && label.startsWith('com.linkos.')) {
      ...
      try {
        execSync('systemctl --user stop "com.linkos.*" 2>/dev/null', { stdio: 'ignore', timeout: 3000 });
      } catch { /* ok */ }
    }
