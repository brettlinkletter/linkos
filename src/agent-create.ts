...
import { LINKOS_CONFIG, PROJECT_ROOT, STORE_DIR } from './config.js';
...
export async function createAgent(opts: CreateAgentOpts): Promise<CreateAgentResult> {
  ...
  // Determine agent directory (prefer LINKOS_CONFIG if it exists)
  let agentDir: string;
  const externalAgentsDir = path.join(LINKOS_CONFIG, 'agents');
  if (fs.existsSync(LINKOS_CONFIG)) {
...
function generateLaunchdPlist(agentId: string): string {
  ...
  const label = `com.linkos.${agentId}`;
...
function generateSystemdUnit(agentId: string): string {
  ...
  const serviceName = `com.linkos.agent-${agentId}`;
  const unitPath = path.join(unitDir, `${serviceName}.service`);

  const unit = `[Unit]
Description=LinkOS Agent: ${agentId}
...
export function deleteAgent(agentId: string): { ok: boolean; error?: string } {
  ...
  try {
    ...
    for (const baseDir of [
      path.join(LINKOS_CONFIG, 'agents'),
    ...
    const plistTemplate = path.join(PROJECT_ROOT, 'launchd', `com.linkos.${agentId}.plist`);
...
export function suggestBotNames(agentId: string): { displayName: string; username: string } {
  ...
  return {
    displayName: `LinkOS ${label}`,
    username: `linkos_${agentId.replace(/-/g, '_')}_bot`,
  };
}
