...
import { downloadMedia, buildPhotoMessage, buildDocumentMessage, buildVideoMessage, normalizeImage, detectImageType } from './media.js';
 ...
 */
async function handleMessage(ctx: Context, message: string, forceVoiceReply = false, skipLog = false, isImageRetry = false): Promise<void> {
  ...
  try {
    ...
    // Baseline of claude subprocesses BEFORE this query, so on timeout we only
    // reap the subtree this query spawns (not a concurrent chat's live query).
    ...
    const timeoutId = setTimeout(() => {
      ...
      abortCtrl.abort();
      // Backstop: the SDK's abort has been observed to leave a wedged `claude`
      // (and orphaned MCP browsers) running, which blocks runAgent from ever
      // returning and freezes the queue. Force-kill the leaked subtree after a
      // short grace so the SDK's stdio closes and runAgent unblocks.
      void reapLeakedSubprocesses(claudeBaseline, { graceMs: 5000 }).then((killed) => {
        if (killed.length) {
          logger.warn({ chatId: chatIdStr, killed }, 'Reaped stuck agent subprocesses after timeout');
        }
      });
    }, AGENT_TIMEOUT_MS);
    ...
    // A real result means the current Claude credential works -- clear any
    // pending auth-restart budget.
    recordAuthSuccess();
    ...
    // Stale shared-OAuth token -> 401. Restart to pick up a fresh token
    // (or, if restarts aren't helping, tell the user to re-auth). If handled,
    // stop here so we don't also send a raw error message.
    if (await handleAuthError(err, async (m) => { await ctx.reply(m); })) {
      return;
    }
    ...
    } else if (/could not process image|image exceeds|unsupported image|invalid.*image|image.*too large/i.test(errMsg)) {
      // A corrupt or undecodable image is baked into the resumed transcript.
      // The Agent SDK replays the full transcript every turn, so once a bad image
      // lands in history EVERY subsequent message 400s — even ones with no image.
      // Clear the session to drop the poisoned transcript, then retry once on a
      // fresh session. If the retry also fails, the bad image is in the current
      // message itself, so we stop instead of looping.
      clearSession(chatIdStr, AGENT_ID);
      logger.warn({ chatId: chatIdStr, agentId: AGENT_ID, isImageRetry }, 'Image processing error — cleared poisoned session');
      if (!isImageRetry) {
        await ctx.reply('Hit an image the API could not decode in the session history. Reset the session and retrying...');
        messageQueue.enqueue(chatIdStr, () => handleMessage(ctx, message, forceVoiceReply, skipLog, true));
      } else {
        await ctx.reply(
          "That image can't be processed (corrupt, or an unsupported format like HEIC/AVIF). The session has been reset. Try a different image, or send it as a document so I can convert it.",
        );
      }
    } else {
...
export function createBot(): Bot {
  ...
  // Reject group chats. LinkOS only works in private (1-on-1) chats.
  ...
  bot.command('start', (ctx) => {
    ...
    return ctx.reply('LinkOS online. What do you need?');
  });
  ...
  bot.on('message:photo', async (ctx) => {
    ...
    try {
      ...
      // Normalise before Claude ever reads it: convert/resize unsupported or
      // oversized images so a bad one can't poison the session transcript.
      const norm = await normalizeImage(localPath);
      if (!norm.path) {
        await ctx.reply(`Can't process that image: ${norm.reason}. Try a different one or send it as a document.`);
        return;
      }
      const msg = buildPhotoMessage(norm.path, ctx.message.caption ?? undefined);
      const chatIdStr = chatId.toString();
  ...
  bot.on('message:document', async (ctx) => {
    ...
    try {
      ...
      // Documents are often images (especially HEIC/AVIF from phones, which
      // Telegram delivers as documents). If this one is an image, normalise it
      // and treat it as a photo so an undecodable image can't poison the session.
      const head = fs.readFileSync(localPath).subarray(0, 32);
      if (detectImageType(head) !== null) {
        const norm = await normalizeImage(localPath);
        if (!norm.path) {
          await ctx.reply(`Can't process that image: ${norm.reason}. Try a different format.`);
          return;
        }
        const msg = buildPhotoMessage(norm.path, ctx.message.caption ?? undefined);
        messageQueue.enqueue(chatIdStr, () => handleMessage(ctx, msg));
...
async function processDashboardMessage(
  ...
  try {
    ...
    const dashTimeout = setTimeout(() => {
      ...
      void reapLeakedSubprocesses(claudeBaseline, { graceMs: 5000 }).then((killed) => {
        if (killed.length) {
          logger.warn({ chatId: chatIdStr, killed }, 'Reaped stuck dashboard agent subprocesses after timeout');
        }
      });
    }, AGENT_TIMEOUT_MS);
