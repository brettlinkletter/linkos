...
LinkOS is not a chatbot wrapper. It spawns the actual `claude` CLI on your Mac or Linux machine and pipes the result back to your Telegram chat. Everything that works in your terminal (your skills, your tools, your context) works from your phone.

---

## What's New in v2.0

LinkOS adds enterprise-grade infrastructure on top of the battle-tested v1 foundation. Zero new runtime dependencies -- everything is built with native APIs.
...
![LinkOS setup flow](assets/setup-flow.jpeg)
...
**Windows (PowerShell):**
```powershell
irm https://raw.githubusercontent.com/brettlinkletter/linkos/main/install.ps1 | iex
```
...
**macOS users:** After starting LinkOS for the first time, your Mac may show "Node wants to access..." permission dialogs. You need to click Allow on each one or the bot will silently hang. Keep an eye on your Mac screen during the first run.

**Which Claude plan works best?** LinkOS runs the `claude` CLI, so any plan works (Free, Pro, Max). However, complex multi-step tasks (building skills, debugging code, multi-agent work) perform significantly better on **Opus**: If you're on the Free or Pro plan and Claude struggles with a task, the model matters. Sonnet is fast but often can't handle the kind of agentic work LinkOS enables. Max ($100 or $200) with Opus is the recommended experience.
...
You need a bot token from Telegram. This is what LinkOS uses to send and receive messages.
...
```bash
git clone https://github.com/brettlinkletter/linkos.git
cd linkos
./setup
```
...
> ```
> I just cloned LinkOS. Please read README.md and set me up completely.
...
```bash
cd linkos          # go to your LinkOS directory
...
![LinkOS architecture](assets/architecture.png)
...
> LinkOS ships with bundled Gmail and Google Calendar skills that work great out of the box. This is an **optional alternative** if you want broader Google Workspace access from a single tool.
...
| **Skill** | A markdown file (`.md`) that teaches Claude how to do something specific, usually by combining CLI commands, API calls, or code into a workflow. | Claude reads the skill file and follows its instructions. LinkOS's bundled Gmail skill, for example, tells Claude which Python scripts to run and how to format the output. |
...
LinkOS downloads the video to `workspace/uploads/` and tells Claude to analyze it with the `gemini-api-dev` skill. Without `GOOGLE_API_KEY`, Claude receives the file path but can't understand the content. Telegram caps downloads at 20MB.
...
**How it works:** On startup, LinkOS scans `~/.claude/skills/` for folders containing a `SKILL.md` with valid YAML frontmatter. If `user_invocable: true` is set, the skill's `name` and `description` are registered with Telegram's `setMyCommands` API alongside the built-in commands. Telegram allows up to 100 commands total.
...
When you start LinkOS, a small web page starts running alongside the bot. It reads directly from the same database the bot uses and shows you the data in real time.
...
Open the `.env` file in your LinkOS folder. (This is the same file where your Telegram token and other keys live. Open it with any text editor. TextEdit on Mac, Notepad on Windows, or whatever your terminal editor is.)
...
cloudflared tunnel create linkos

# 4. Connect your domain to the tunnel (replace with your actual domain)
cloudflared tunnel route dns linkos dash.yourdomain.com
```
...
**Moving to a new machine later?** Copy two files from the old machine: `~/.cloudflared/config.yml` and the `.json` credentials file next to it. Run `cloudflared tunnel run linkos` on the new machine. Same URL, no changes needed.
...
Requires a Slack User OAuth Token. This connects to your workspace so LinkOS can read and send messages on your behalf.
...
4. Fill in:
   - **App Name**: anything you want (e.g. `LinkOS`)
...
### Step 4: Add the token to LinkOS

1. Open your `.env` file in the LinkOS project directory
2. Add the token:
   ```
...
- You restarted LinkOS after adding the token

### Using Slack from Claude Code (skill)

LinkOS ships with a Slack CLI at `dist/slack-cli.js` and a matching skill in `skills/slack/`. To use Slack via natural language from any Claude Code session:
...
All WhatsApp message bodies are **encrypted at rest** using AES-256-GCM before being written to the database. Even if someone accesses `store/linkos.db` directly, message content is unreadable without the encryption key in your `.env`.
...
![LinkOS memory system diagram](assets/memory-diagram.png)

LinkOS has a structured memory system that extracts, consolidates, and recalls knowledge across all sessions. Everything is automatic.
...
LinkOS ships with SQLite and **creates everything automatically on first run**: No migrations, no setup, no external database server. File lives at `store/linkos.db`.
...
`CLAUDE.md` is loaded into every Claude Code session. It's the personality and context file. the main thing to edit to make LinkOS yours.
...
LinkOS auto-loads every skill in `~/.claude/skills/`. Install a skill by copying its folder there.

### Bundled skills

LinkOS ships with ready-to-use skills in the `skills/` directory. Copy any of these to activate them:
...
**Yes.** On February 19, 2026, Anthropic published their [Legal and Compliance page](https://docs.anthropic.com/en/docs/legal-and-compliance) banning OAuth tokens in third-party tools. LinkOS is not affected.
...
**Why LinkOS is different:** `@anthropic-ai/claude-agent-sdk`'s `query()` spawns the `claude` binary as a child process. That subprocess manages its own OAuth from `~/.claude/`. LinkOS never reads or transmits your token. It runs Claude Code and reads the output. identical to `claude -p "message"` in a terminal.

| | LinkOS | Token-extraction tools |
|---|---|---|
...
LinkOS has multiple security layers. Some are always on, others are opt-in. The setup wizard (`npm run setup`) configures all of them interactively.

### Always on

These protections are active in every LinkOS installation, no configuration needed.
...
Set `EMERGENCY_KILL_PHRASE` to a unique phrase. Sending it immediately stops all LinkOS launchd/systemd services and force-exits the process. This is a hard stop, not a lock. Use it if something goes wrong and you need everything shut down now.
...
- Check logs: `tail -f /tmp/linkos.log`
...
- You do **not** need to fill in every bracket. At minimum: `[YOUR ASSISTANT NAME]`, `[YOUR NAME]`, and `[PATH TO LINKOS]` (the full path to your linkos directory)
...
- LinkOS runs the real Claude Code CLI, which has full internet access through its built-in tools (web search, web fetch, bash with curl, etc.)
- If Claude says it can't access the internet, it's likely a skill or prompt issue, not a LinkOS limitation
...
- Two instances running. Kill the old one: `kill $(cat store/linkos.pid)`

**Session feels off or confused**
- Send `/newchat` for a fresh start

**File downloads fail**
- Telegram caps downloads at 20MB. this is a Telegram API limit, not a LinkOS one
...
No. LinkOS has nothing to do with Anthropic's Remote product. It runs the `claude` CLI locally on your own machine (Mac, Linux, or Windows via WSL2) and pipes results to Telegram. No cloud VMs, no remote sessions.

**"Does this work on Windows?"**
Yes, through WSL2. Install WSL2, clone LinkOS inside the WSL filesystem, and follow the normal Linux setup steps. The setup wizard detects Windows and offers WSL2 or PM2 options.
...
Recommended but not required. The video covers how Claude Code works under the hood, which helps you understand what LinkOS is actually doing. But you can set up LinkOS first and watch it later.

**"How do I update when a new version drops?"**
`cd linkos && git pull && npm install && npm run migrate && npm run build` then restart. See [Updating LinkOS](#updating-linkos) above.

**"Telegram formatting looks broken / not formatting properly"**
LinkOS converts Claude's Markdown to Telegram-safe HTML (bold, italic, code blocks, links). Telegram's formatting support is limited compared to a full web page. If something looks off, it's usually Telegram's rendering, not a bug. For very long or complex responses, the formatting is intentionally kept simple to avoid Telegram parse errors.
...
├── linkos.plist      macOS LaunchAgent template (setup wizard uses this)
...
├── store/
│   ├── linkos.db     SQLite database. created automatically on first run
│   ├── linkos.pid    Tracks the running process to prevent duplicates
...
LinkOS can run **specialist agents** alongside the main bot. Each agent is its own Telegram bot with its own personality, its own Claude Code session, and its own chat on your phone.
...
Your main LinkOS bot does everything. That's powerful but also means one long conversation, one context window, and one personality trying to handle email, research, billing, and content all at once.
...
Each will show:
```
LinkOS agent [comms] online: @yourname_comms_bot
```
...
```
com.linkos.main:     running (PID: 12345)
com.linkos.comms:    running (PID: 12346)
com.linkos.content:  running (PID: 12347)
com.linkos.ops:      running (PID: 12348)
com.linkos.research: running (PID: 12349)
...
launchctl unload ~/Library/LaunchAgents/com.linkos.comms.plist
launchctl load ~/Library/LaunchAgents/com.linkos.comms.plist

# Restart ALL agents after a rebuild
npm run build
for agent in main comms ops content research; do
  launchctl unload ~/Library/LaunchAgents/com.linkos.$agent.plist 2>/dev/null
  launchctl load ~/Library/LaunchAgents/com.linkos.$agent.plist
done
