...
allowed-tools: Bash(LINKOS_DIR=* ~/.venv/bin/python3 ~/.config/gmail/gmail.py *)
---
...
The Gmail CLI reads credential paths from environment variables, loaded from LinkOS's `.env` via `LINKOS_DIR`. Every command MUST use this prefix:

```
LINKOS_DIR=/path/to/linkos
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py list --all
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py list --hours 48
```

### Read full email

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py read <msg_id>
```

### Move email to label/folder

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py move <msg_id> "Label Name"
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py labels
```

### Reply to an email

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py reply <msg_id> "Your reply body here"
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py reply <msg_id> "Your reply body here" --attachments "/path/to/file1.pdf,/path/to/file2.png"
```

### Send a new email

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py send "to@example.com" "Subject here" "Body here"
```

### Send with attachments

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py send "to@example.com" "Subject" "Body" --attachments "/path/to/file.pdf,/path/to/other.xlsx"
```

### Create a Gmail filter (auto-sort rule)

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py filter --from "sender@example.com" --label "LabelName" --archive --read
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py filters
```

### Re-authenticate

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/gmail/gmail.py auth
```
