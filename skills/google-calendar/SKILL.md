...
allowed-tools: Bash(LINKOS_DIR=* ~/.venv/bin/python3 ~/.config/calendar/gcal.py *)
---
...
The calendar CLI reads credential paths from environment variables, loaded from LinkOS's `.env` via `LINKOS_DIR`. Every command MUST use this prefix:
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py list
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py list --days 7
```

### Get event details

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py get <event_id>
```

### Create event with Meet link and invites

```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py create "Meeting Title" "2026-03-15 10:00" --duration 30 --attendees "person@example.com,other@example.com" --meet
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py update <event_id> --title "New Title" --start "2026-03-16 14:00" --duration 60 --add-attendees "new@example.com" --meet
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py cancel <event_id>
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py freebusy "2026-03-15 09:00" "2026-03-15 17:00"
```
...
```bash
LINKOS_DIR=/path/to/linkos ~/.venv/bin/python3 ~/.config/calendar/gcal.py auth
```
