# RFC: SDK Engine — Direct API Backend for LinkOS
...
giving LinkOS direct control over the agentic loop, tool execution, token accounting,
...
### 3. Tighter integration with LinkOS internals

The CLI subprocess runs in its own process with its own environment. Passing secrets
requires `env` injection (`sdkEnv` in `agent.ts:117-123`). Reading LinkOS's SQLite
...
- **Custom tool schemas**: define LinkOS-specific tools (e.g., `MemorySearch`,
...
1. **Session management is opaque**: The CLI manages session files internally.
   LinkOS only stores the `session_id` string in SQLite (`sessions` table)
   and passes it back via `resume`.

2. **Tool execution is invisible**: LinkOS sees `tool_progress` events with
...
4. **System prompt is implicit**: CLAUDE.md is loaded by the CLI from `cwd`.
   LinkOS's `agentSystemPrompt` is prepended to the user message, not
...
The CLI engine manages sessions opaquely — LinkOS only stores the session ID.
...
export class SessionStore {
  ...
  constructor(dbPath?: string) {
    // Uses a separate DB file to avoid schema conflicts with the main linkos.db.
...
| 6 | **In-process tool access** | Tools can call `db.ts` functions directly, read LinkOS state |
...
Since LinkOS is a personal bot on a trusted machine with `ALLOWED_CHAT_ID` filtering,
...
Future option: support a subset — e.g., MCP servers defined in a LinkOS config file
...
LinkOS already disables the CLI's permission system (`permissionMode: 'bypassPermissions'`).
The SdkEngine would run tools with the same user privileges. Bash commands run as the
LinkOS process user. File operations have full filesystem access.
...
different formats. The `sessions` table in `linkos.db` stores the session ID
...
Run the `claude` CLI in server mode, call it via HTTP from LinkOS.
...
A typical LinkOS turn with Opus (200k context, 2k output) costs ~$3.15.
