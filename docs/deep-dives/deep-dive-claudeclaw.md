...
**Date:** 2026-03-30
**Source:** `linkos/product/linkos/` (46 TypeScript source files, ~4,500 lines)
**Old v1 repo:** `linkos/other/old-repo/linkos-v1/` (shell/JS installer + compiled dist)
...
4. Create directory under `LINKOS_CONFIG/agents/<id>/` (external) or `agents/<id>/` (repo)
...
Agents are looked up in two locations with LINKOS_CONFIG taking priority:
```
LINKOS_CONFIG/agents/<id>/agent.yaml   (preferred -- outside repo)
...
Agent ID auto-detected from `LINKOS_AGENT_ID` env var (set by `index.ts`).
...
Personal config lives outside the repo in `LINKOS_CONFIG` (default `~/.linkos`):
...
Message received (any bot, any state)
  ...
  -> Stop all launchd/systemd services matching com.linkos.*
...
The two-location lookup (LINKOS_CONFIG/agents/ -> PROJECT_ROOT/agents/) allows personal config to override repo defaults without modifying tracked files.
...
13. **No config-driven templating**: V1's `linkos.config.json` + template rendering (`{{business.name}}`) system is gone. V2 requires manual CLAUDE.md editing.
