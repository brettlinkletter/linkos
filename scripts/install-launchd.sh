#!/bin/bash
# Install LinkOS launchd agents for auto-start on login + auto-restart on crash
set -e
...
# Clean up stale/orphaned linkos agents not in the current launchd/ directory
echo "Cleaning up stale agents..."
for existing in "$LAUNCH_AGENTS_DIR"/com.linkos.*.plist; do
  [ -f "$existing" ] || continue
...
# Also remove the bare com.linkos.plist (legacy, pre-multi-agent)
if [ -f "$LAUNCH_AGENTS_DIR/com.linkos.plist" ]; then
  echo "  Removing legacy agent: com.linkos"
  launchctl unload "$LAUNCH_AGENTS_DIR/com.linkos.plist" 2>/dev/null || true
  rm -f "$LAUNCH_AGENTS_DIR/com.linkos.plist"
fi
echo ""
...
# Install each plist
for plist in "$LAUNCHD_DIR"/com.linkos.*.plist; do
...
if $all_ok; then
  ...
  echo "  launchctl list | grep linkos    # check status"
