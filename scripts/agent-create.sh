...
# Resolve config directory — LINKOS_CONFIG from .env or environment, default ~/.linkos
if [ -z "$LINKOS_CONFIG" ]; then
  LINKOS_CONFIG=$(grep '^LINKOS_CONFIG=' .env 2>/dev/null | cut -d'=' -f2- | sed "s|^~|$HOME|")
fi
LINKOS_CONFIG="${LINKOS_CONFIG:-$HOME/.linkos}"

echo "=== LinkOS Agent Creator ==="
echo ""
...
# Config goes to LINKOS_CONFIG if the dir exists, otherwise repo's agents/
if [ -d "$LINKOS_CONFIG" ]; then
  AGENT_DIR="$LINKOS_CONFIG/agents/$AGENT_ID"
...
echo "  3. Name it something like 'Mark $AGENT_LABEL' or 'LinkOS $AGENT_LABEL'"
