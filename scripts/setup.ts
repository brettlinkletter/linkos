...
async function main() {
  ...
  console.log(`  ${c.bold}Welcome to LinkOS.${c.reset}`);
  console.log();
  ...
  info('LinkOS has several optional features. Tell us what you want.');
  ...
  if (wantWhatsApp) {
    ...
    console.log();
    info('LinkOS uses whatsapp-web.js to connect to your existing WhatsApp');
  ...
  // ── 5. Config directory (LINKOS_CONFIG) ──────────────────────────────
  section('Config directory (LINKOS_CONFIG)');

  info('Personal config files (CLAUDE.md, agent configs) live outside the repo');
  info('so they are never accidentally committed. Defaults to ~/.linkos');
  console.log();

  const envForConfig = parseEnvFile(path.join(PROJECT_ROOT, '.env'));
  const defaultConfigDir = expandHome(
    envForConfig.LINKOS_CONFIG || '~/.linkos',
  );
  info(`Current path: ${defaultConfigDir}`);
  console.log();

  let linkosConfigDir = defaultConfigDir;
  const changeConfigDir = await confirm('Change this path?', false);
  if (changeConfigDir) {
    const input = await ask('Config directory', '~/.linkos');
    linkosConfigDir = expandHome(input.trim() || '~/.linkos');
  }

  // If the chosen directory already exists, notify and let user decide
  if (fs.existsSync(linkosConfigDir)) {
    const hasClaudeMd = fs.existsSync(path.join(linkosConfigDir, 'CLAUDE.md'));
    ...
    if (!useExisting) {
      ...
      if (newPath.trim()) linkosConfigDir = expandHome(newPath.trim());
    }
  }

  // Create the directory if needed
  if (!fs.existsSync(linkosConfigDir)) {
    fs.mkdirSync(linkosConfigDir, { recursive: true });
    ok(`Created ${linkosConfigDir}`);
  }

  // Ensure CLAUDE.md exists in the config dir (copy from example if needed)
  const claudeMdDest = path.join(linkosConfigDir, 'CLAUDE.md');
  ...
  info('LinkOS auto-loads every skill in ~/.claude/skills/.');
  ...
  if (wantVideo) {
    ...
    console.log();
    info('LinkOS\'s video analysis uses the gemini-api-dev skill from Google.');
  ...
  // Persist LINKOS_CONFIG determined in section 6
  env.LINKOS_CONFIG = linkosConfigDir;
  ...
  if (wantSlack) {
    ...
    console.log();
    info('LinkOS uses a Slack User Token to read and post messages');
  ...
  info('LinkOS has full access to your machine. If someone gets into');
  ...
  const lines = [
    '# LinkOS — generated by setup wizard',
    ...
    `LINKOS_CONFIG=${env.LINKOS_CONFIG || ''}`,
    '',
  ...
  if (PLATFORM === 'darwin') {
    ...
    info('Or use PM2: pm2 start dist/index.js --name linkos && pm2 save');
  }
  ...
  info('LinkOS can run specialist agents alongside the main bot.');
  ...
  console.log(`  ${c.cyan}║${c.reset}${c.bold}           LinkOS is ready!             ${c.reset}${c.cyan}║${c.reset}`);
  console.log(`  ${c.cyan}╚════════════════════════════════════════════╝${c.reset}`);
  console.log();
  ...
  if (tsInstalled) {
    ...
    if (tsIp) {
      ...
      const addTeam = await confirm('Share SSH access with your team?', true);
      if (addTeam) {
        console.log();
  ...
  if (PLATFORM === 'darwin') {
    info('Logs: tail -f /tmp/linkos.log');
  } else if (PLATFORM === 'linux') {
    info('Logs: journalctl --user -u linkos -f');
  }
  console.log();
...
async function setupMacOS() {
  ...
  const dest = path.join(os.homedir(), 'Library', 'LaunchAgents', 'com.linkos.app.plist');
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.linkos.app</string>
  ...
  <key>StandardOutPath</key><string>/tmp/linkos.log</string>
  <key>StandardErrorPath</key><string>/tmp/linkos.err</string>
...
async function setupLinux() {
  ...
  if (!install) {
    ...
    info('Or: pm2 start dist/index.js --name linkos && pm2 save');
    return;
  }
  ...
  try {
    ...
    const servicePath = path.join(serviceDir, 'linkos.service');
    const service = `[Unit]
Description=LinkOS Telegram Bot
...
`;
    ...
    execSync('systemctl --user enable linkos', { stdio: 'pipe' });
    execSync('systemctl --user start linkos', { stdio: 'pipe' });
...
function setupWindows() {
  ...
  info('  Install WSL2, clone LinkOS inside the WSL2 filesystem,');
  ...
  console.log(`  ${c.cyan}pm2 start dist/index.js --name linkos${c.reset}`);
