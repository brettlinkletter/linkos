...
async function main() {
  console.log();
  console.log(`  ${c.bold}${c.cyan}LinkOS Status${c.reset}`);
  ...
  if (process.platform === 'darwin') {
    try {
      const output = execSync('launchctl list com.linkos.app', {
        stdio: 'pipe',
      })
        .toString()
        .trim();
      ...
      if (pidMatch) {
        ...
        for (const line of lines) {
          ...
          if (parts.length >= 3 && parts[2] === 'com.linkos.app') {
            pid = parts[0].trim();
            break;
          }
        }
    ...
    try {
      const output = execSync('systemctl --user is-active linkos', {
        stdio: 'pipe',
      }).toString().trim();
      if (output === 'active') {
    ...
    try {
      execSync('pm2 describe linkos', { stdio: 'pipe' });
  ...
  // Memory DB
  const dbPath = path.join(PROJECT_ROOT, 'store', 'linkos.db');
